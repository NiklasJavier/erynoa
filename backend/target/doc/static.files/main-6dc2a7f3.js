...
function showMain() {
	...
	if (mainHeading && window.searchState.rustdocToolbar) {
		if (window.searchState.rustdocToolbar.parentElement) {
			window.searchState.rustdocToolbar.parentElement.removeChild(window.searchState.rustdocToolbar)
		}
...
;(() => {
	...
	window.searchState = {
		...
		isDisplayed: () => {
			...
			return !!container.parentElement && container.parentElement.id === ALTERNATIVE_DISPLAY_ID
		},
		focus: () => {
	...
	function handleHashes(ev) {
		...
		if (savedHash.startsWith('impl-')) {
			...
			if (splitAt !== -1) {
				...
				const implElems = document.querySelectorAll(`details > summary > section[id^="${implId}"]`)
	...
	window.register_type_impls = (imp) => {
		...
		for (const impList of imp[window.currentCrate]) {
			...
			if (outputList === null) {
				...
				const outputListId = isTrait ? 'trait-implementations-list' : 'implementations-list'
			...
			const templateAssocItems = template.content.querySelectorAll(
				'section.tymethod, ' + 'section.method, section.associatedtype, section.associatedconstant'
			)
			if (isTrait) {
				const li = document.createElement('li')
	...
	function showTooltip(e) {
		...
		if (finalPos > 0) {
			...
			wrapper.style.setProperty('--popover-arrow-offset', wrapperPos.right - pos.right + 4 + 'px')
		}
	...
	function setTooltipHoverTimeout(element, show) {
		...
		if (window.CURRENT_TOOLTIP_ELEMENT && window.CURRENT_TOOLTIP_ELEMENT.TOOLTIP_BASE !== element) {
			return
		}

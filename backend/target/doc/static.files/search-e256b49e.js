const initSearch = async (Stringdex, RoaringBitmap, hooks) => {
	...
	function isLastElemGeneric(elems, parserState) {
		return (
			(elems.length > 0 && elems[elems.length - 1].generics.length > 0) || prevIs(parserState, '>')
		)
	}
	...
	function getItemsBefore(query, parserState, elems, endChar) {
		...
		while (parserState.pos < parserState.length) {
			...
			if (!foundStopChar) {
				...
				if (endChar !== '') {
					throw ['Expected ', ',', ', ', '=', ', or ', endChar, ...extra, ', found ', c]
				}
		...
		if ('[('.indexOf(parserState.userQuery[parserState.pos]) !== -1) {
			...
			if (name === '()' && !foundSeparator && generics.length === 1 && typeFilter === null) {
				...
				if (typeFilter !== null && typeFilter !== 'primitive') {
					throw ['Invalid search type: primitive ', name, ' and ', typeFilter, ' both specified']
				}
				parserState.totalElems += 1
				if (isInGenerics) {
	...
	class DocSearch {
			...
			try {
				...
				const checkTypeFilter = (elem) => {
					...
					if (ty === TY_GENERIC && elem.generics.length !== 0) {
						throw ['Generic type parameter ', elem.name, ' does not accept generic parameters']
					}
		...
		async getFunctionData(id) {
			...
			const parser = new VlqHexDecoder(raw[0], async (functionSearchType) => {
				...
				if (typeof functionSearchType[INPUTS_DATA] === 'number') {
					inputs_ = Promise.all([this.buildItemSearchType(functionSearchType[INPUTS_DATA])])
				} else {
					inputs_ = this.buildItemSearchTypeAll(functionSearchType[INPUTS_DATA])
				}
				if (functionSearchType.length > 1) {
					if (typeof functionSearchType[OUTPUT_DATA] === 'number') {
						output_ = Promise.all([this.buildItemSearchType(functionSearchType[OUTPUT_DATA])])
					} else {
		...
		async getRow(id, loadFunctionData) {
			...
			const [
				...
				entry && entry.exactModulePath !== null ? this.getName(entry.exactModulePath) : null,
				entry && entry.exactModulePath !== null ? this.getPathData(entry.exactModulePath) : null,
				entry && entry.parent !== null ? this.getName(entry.parent) : null,
				entry && entry.parent !== null ? this.getPathData(entry.parent) : null,
				entry ? nonnull(await this.getName(entry.krate)) : '',
			])
			const name = name_ === null ? '' : name_
			const normalizedName = (
				name.indexOf('_') === -1 ? name : name.replace(/_/g, '')
			).toLowerCase()
			const modulePath =
				modulePathData === null || moduleName === null
					? ''
			...
			return {
				...
				modulePath,
				exactModulePath:
					exactModulePathData === null || exactModuleName === null
						? modulePath
			...
			if (typeof type === 'number') {
				...
				if (type[BINDINGS_DATA] && type[BINDINGS_DATA].length > 0) {
					bindings = new Map(
						(
							await Promise.all(
								type[BINDINGS_DATA].map(async (binding) => {
		...
		async execQuery(parsedQuery, filterCrates, currentCrate) {
			...
			const buildHrefAndPath = (item) => {
				...
				if (type === 'mod') {
					...
					href =
						this.rootPath + item.modulePath.replace(/::/g, '/') + '/index.html#reexport.' + name
				} else if (type === 'primitive' || type === 'keyword' || type === 'attribute') {
					displayPath = ''
					exactPath = ''
					href = this.rootPath + path.replace(/::/g, '/') + '/' + type + '.' + name + '.html'
					...
					displayPath = item.modulePath + '::'
					href =
						this.rootPath + item.modulePath.replace(/::/g, '/') + '/' + type + '.' + name + '.html'
				}
			...
			const formatDisplayTypeSignature = async (obj, typeInfo, elems, returned) => {
				...
				const writeHof = async (fnType, result) => {
					...
					pushText({ name: hofOutput.length === 0 ? ')' : ') -> ', highlighted: false }, result)
				...
				const writeSpecialPrimitive = async (fnType, result) => {
					if (
						...
						await onEachBtwnAsync(
							...
							(value) =>
								pushText({ name: ' ', highlighted: prevHighlighted && value.highlighted }, result)
						)
						return true
				...
				const writeFn = async (fnType, result) => {
					if (fnType.id !== null && fnType.id < 0) {
						if (fnParamNames[-1 - fnType.id] === '') {
							const generics =
								fnType.generics.length > 0 ? fnType.generics : objType.where_clause[-1 - fnType.id]
							...
							for (const [queryId, fnId] of mgens) {
								if (fnId === fnType.id) {
									mappedNames.set(queryParamNames[-1 - queryId], fnParamNames[-1 - fnType.id])
								}
							}
						}
						pushText(
							{ name: fnParamNames[-1 - fnType.id], highlighted: !!fnType.highlighted },
							result
						)
						const where = []
						await onEachBtwnAsync(
						...
						if (fnType.bindings.size > 0) {
							await onEachBtwnAsync(
								await Promise.all(
									[...fnType.bindings.entries()].map(async ([key, values]) => [
										await this.getName(key),
										values,
									])
								),
								async ([name, values]) => {
									...
									pushText({ name, highlighted: false }, result)
									pushText({ name: values.length !== 1 ? '=(' : '=', highlighted: false }, result)
									await onEachBtwnAsync(
										values || [],
										(value) => writeFn(value, result),
										() => pushText({ name: ' + ', highlighted: false }, result)
						...
						const obj = Object.assign(
							{
								parent: item.parent
									...
									: undefined,
								type:
									item.functionData && item.functionData.functionSignature
										? item.functionData.functionSignature
										: undefined,
								paramNames:
									item.functionData && item.functionData.paramNames
			...
			const sortAndTransformResults = async function* (
				results,
				typeInfo,
				preferredCrate,
				duplicates
			...
			function unifyFunctionTypes(
				...
				if (ql === 1 && queryElems[0].generics.length === 0 && queryElems[0].bindings.size === 0) {
					const queryElem = queryElems[0]
					for (const [i, fnType] of fnTypesIn.entries()) {
						if (!unifyFunctionTypeIsMatchCandidate(fnType, queryElem, mgens, typeNameIds)) {
							continue
						}
						if (fnType.id !== null && fnType.id < 0 && queryElem.id !== null && queryElem.id < 0) {
							if (mgens && mgens.has(queryElem.id) && mgens.get(queryElem.id) !== fnType.id) {
								continue
							}
				...
				for (let i = flast; i >= 0; i -= 1) {
					...
					if (fnType.id !== null && queryElem.id !== null && fnType.id < 0) {
						...
						if (mgensScratch.has(queryElem.id) && mgensScratch.get(queryElem.id) !== fnType.id) {
							continue
						}
						mgensScratch.set(queryElem.id, fnType.id)
					} else {
				...
				for (let i = flast; i >= 0; i -= 1) {
					...
					const generics =
						fnType.id !== null && fnType.id < 0 ? whereClause[-fnType.id - 1] : fnType.generics
					const bindings = fnType.bindings ? Array.from(fnType.bindings.values()).flat() : []
					const passesUnification = unifyFunctionTypes(
			...
			function unifyGenericTypes(
				...
				if (
					...
					if (fnType.id !== null && fnType.id < 0) {
						...
						if (highlightedGenerics) {
							return [
								Object.assign({ highlighted: true }, fnType, { generics: highlightedGenerics }),
			...
			const unifyFunctionTypeIsMatchCandidate = (fnType, queryElem, mgensIn, typeNameIds) => {
				...
				if (fnType.id !== null && fnType.id < 0 && queryElem.id !== null && queryElem.id < 0) {
					if (mgensIn && mgensIn.has(queryElem.id) && mgensIn.get(queryElem.id) !== fnType.id) {
						return false
					}
					return true
				} else {
					if (
						queryElem.id === typeNameIds.typeNameIdOfArrayOrSlice &&
						(fnType.id === typeNameIds.typeNameIdOfSlice ||
							fnType.id === typeNameIds.typeNameIdOfArray)
					) {
					} else if (
						queryElem.id === typeNameIds.typeNameIdOfTupleOrUnit &&
						(fnType.id === typeNameIds.typeNameIdOfTuple ||
							fnType.id === typeNameIds.typeNameIdOfUnit)
					) {
					} else if (
						queryElem.id === typeNameIds.typeNameIdOfHof &&
						(fnType.id === typeNameIds.typeNameIdOfFn ||
							fnType.id === typeNameIds.typeNameIdOfFnMut ||
							fnType.id === typeNameIds.typeNameIdOfFnOnce ||
							fnType.id === typeNameIds.typeNameIdOfFnPtr)
					...
					if (queryElemPathLength > 0) {
						const fnTypePath =
							fnType.path !== undefined && fnType.path !== null ? fnType.path.split('::') : []
			...
			function unifyFunctionTypeIsUnboxCandidate(
				...
				} else if (fnType.unboxFlag && (fnType.generics.length > 0 || fnType.bindings.size > 0)) {
			...
			function checkPath(contains, path) {
				...
				const maxPathEditDistance = parsedQuery.literalSearch
					? 0
					: Math.floor(contains.reduce((acc, next) => acc + next.length, 0) / 3)
			...
			const innerRunNameQuery = async function* (currentCrate) {
				...
				let count = 0
				const prefixResults = []
				const normalizedUserQuery = parsedQuery.userQuery.replace(/[_"]/g, '').toLowerCase()
				...
				if (aliasResults) {
					for (const id of aliasResults.matches().entries()) {
						const [name, alias] = await Promise.all([this.getName(id), this.getAliasTarget(id)])
						if (
							name !== null &&
							alias !== null &&
							!idDuplicates.has(id) &&
				...
				const handleNameSearch = async (id) => {
					...
					if (
						!typePassesFilter(typeFilter, row.ty) ||
			...
			const innerRunTypeQuery = async function* (inputs, output, typeInfo, currentCrate) {
				const index = this.database.getData('normalizedName')
				if (!index) {
					return
				}
				...
				const everything_postings_list = { invertedIndex: everything_inverted_index, queryElem: [] }
				...
				const unpackPostingsList = async (elem, polarity) => {
					...
					if (typeFilter !== TY_GENERIC && searchResults) {
						for (const id of searchResults.matches().entries()) {
							typePromises.push(
								Promise.all([this.getName(id), this.getTypeData(id), this.getPathData(id)]).then(
									([name, typeData, pathData]) => [id, name, typeData, pathData]
								)
							)
						}
					}
					const types = (await Promise.all(typePromises)).filter(
						([_id, name, ty, path]) =>
							name !== null &&
							name.toLowerCase() === elem.pathLast &&
							ty &&
							...
							(elem.pathWithoutLast.length === 0 ||
								checkPath(elem.pathWithoutLast, path.modulePath.split('::')) === 0)
					)
					if (types.length === 0) {
						const areGenericsAllowed =
							...
							(typeFilter === -1 &&
								...
								elem.generics.length === 0 &&
								elem.bindings.size === 0)
						if (typeFilter !== TY_GENERIC && (elem.name.length >= 3 || !areGenericsAllowed)) {
							let chosenName = null
							let chosenType = []
							let chosenPath = []
							let chosenId = []
							...
							for await (const searchResults of levResults) {
								for (const id of searchResults.matches().entries()) {
									...
									if (
										...
										let dist = editDistance(name, elem.pathLast, maxEditDistance)
										if (elem.pathWithoutLast.length !== 0) {
											const pathDist = checkPath(elem.pathWithoutLast, path.modulePath.split('::'))
											dist += pathDist === null ? Number.MAX_SAFE_INTEGER : pathDist
										}
										if (name === chosenName) {
											chosenId.push(id)
											chosenType.push(ty)
							...
							if (areGenericsAllowed) {
								...
								for (let i = 0; i < chosenType.length; ++i) {
									types.push([chosenId[i], chosenName, chosenType[i], chosenPath[i]])
								}
							}
						}
						if (areGenericsAllowed) {
					...
					for (const [id, _name, typeData] of types) {
						if (
							!typeData ||
							typeData[polarity].every((bitmap) => {
								return bitmap.isEmpty()
							})
						) {
							continue
				...
				const unpackPostingsListAll = async (elems, polarity) => {
					...
					const results = []
					for (const { invertedIndex: firstIdx, queryElem: firstElem } of firstPostingsList) {
						for (const { invertedIndex: remainingIdx, queryElem: remainingElems } of remainingAll) {
							results.push({
				...
				const unpackPostingsListBindings = async (elems, polarity) => {
					if (!elems) {
						return [{ invertedIndex: everything_inverted_index, queryElem: new Map() }]
					}
					...
					if (!firstKeyIds) {
						...
						return [{ invertedIndex: empty_inverted_index, queryElem: new Map() }]
					}
					const results = []
				...
				for (const { invertedIndex: inputsIdx, queryElem: inputs } of allInputs) {
					for (const { invertedIndex: outputIdx, queryElem: output } of allOutput) {
						...
						for (const [size, bitmap] of invertedIndex.entries()) {
							...
							queryPlan[size].push({ bitmap, inputs, output })
						}
					}
				}
				...
				plan: for (const queryStep of queryPlan) {
					for (const { bitmap, inputs, output } of queryStep) {
						for (const id of bitmap.entries()) {
							...
							resultPromises.push(
								this.getFunctionData(id).then(async (fnData) => {
			...
			if (parsedQuery.foundElems === 1 && !parsedQuery.hasReturnArrow) {
				...
				return {
					...
					returned: (async function* () {})(),
					others:
						parsedQuery.foundElems === 0
							? (async function* () {})()
							: innerRunTypeQuery(parsedQuery.elems, parsedQuery.returned, typeInfo, currentCrate),
					query: parsedQuery,
				}
			}
		}
	...
	async function addTab(results, query, display, finishedCallback, isTypeSearch) {
		...
		const addNextResultToOutput = async (obj) => {
			...
			resultName.insertAdjacentHTML('beforeend', `<span class="typename">${typeName}</span>`)
...
${obj.displayPath}<span class="${type}">${name}</span>\
		...
		if (query.correction !== null && isTypeSearch) {
			const orig = query.returned.length > 0 ? query.returned[0].name : query.elems[0].name
			correctionOutput =
		...
		if (firstResult.value) {
			...
			output.innerHTML +=
				...
				'<li><a href="' +
				getNakedUrl() +
				'?search=std::vec">std::vec</a></li>' +
				'<li><a href="' +
				getNakedUrl() +
				'?search=u32+->+bool">u32 -> bool</a></li>' +
				'<li><a href="' +
				getNakedUrl() +
				'?search=Option<T>,+(T+->+U)+->+Option<U>">' +
	...
	async function showResults(docSearch, results, goToFirst, filterCrates) {
		...
		if (window.searchState.rustdocToolbar) {
			nonnull(
				nonnull(window.searchState.containerElement()).querySelector('.main-heading')
	...
	if (typeof window !== 'undefined') {
		...
		onEachLazy(document.querySelectorAll('.search-form.loading'), (form) => {

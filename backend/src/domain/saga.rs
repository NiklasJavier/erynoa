...
mod tests {
    ...
    fn test_intent_creation() {
        ...
        .with_constraint(Constraint::MinTrust { value: 0.5 });
    ...
    fn test_saga_from_intent() {
        ...
        let saga = Saga::from_intent(&intent, steps);
    ...
    #[test]
    fn test_step_with_realm_crossing() {
        let step = SagaStep::new(
            0,
            "Gateway check",
            SagaAction::GatewayCheck {
                did: DID::new_self("alice"),
                target_realm: RealmId::new("realm:erynoa:finance"),
            },
        )
        .with_realm_crossing(
        ...
        assert!(step.realm_crossing.is_some());
    }
}

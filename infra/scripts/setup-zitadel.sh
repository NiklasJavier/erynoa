...
if [ -n "$CLIENT_ID" ] && [ "$CLIENT_ID" != "null" ]; then
    ...
    RESPONSE=$(curl -sf -X POST "${ZITADEL_URL}/management/v1/projects/${PROJECT_ID}/apps/oidc" \
        ...
        -d '{
            ...
            "redirectUris":["http://localhost:5173/callback","http://localhost:5174/callback","http://localhost:3000/callback"],
            "postLogoutRedirectUris":["http://localhost:5173","http://localhost:5174","http://localhost:3000"],
...
if [ -f "$BACKEND_CONFIG" ]; then
    if ! grep -q "frontend_client_id" "$BACKEND_CONFIG" 2>/dev/null; then
        cat >> "$BACKEND_CONFIG" << EOF

# Auto-generated by ZITADEL Setup
[auth]
issuer = "http://localhost:8080"
frontend_client_id = "${CLIENT_ID}"
EOF
        log_ok "Backend config aktualisiert"
    fi
fi

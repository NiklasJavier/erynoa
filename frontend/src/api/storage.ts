...
export class StorageClient {
  ...
  async upload(
    ...
    const endpoint = bucket
      ? `${this.baseUrl}/api/v1/storage/upload/${bucket}`
      : `${this.baseUrl}/api/v1/storage/upload`;
  ...
  async getPresignedUploadUrl(
    ...
    const response = await fetch(
      `${this.baseUrl}/api/v1/storage/presigned/upload/${key}?${params}`,
      { headers }
    );
  ...
  async getPresignedDownloadUrl(
    ...
    const response = await fetch(
      `${this.baseUrl}/api/v1/storage/presigned/download/${key}?${params}`,
      { headers }
    );
  ...
  async list(prefix?: string, bucket?: string): Promise<ListObjectsResponse> {
    ...
    const response = await fetch(
      `${this.baseUrl}/api/v1/storage/list?${params}`,
      { headers }
    );
  ...
  async delete(key: string, bucket?: string): Promise<void> {
    ...
    const response = await fetch(
      `${this.baseUrl}/api/v1/storage/${key}?${params}`,
...
export function initStorageClient(getToken: () => Promise<string | null>) {
  storageClient = new StorageClient(API_BASE_URL, getToken);
...
export function getStorageClient(): StorageClient {
  if (!storageClient) {
    ...
    return new StorageClient(API_BASE_URL, async () => null);
  }
  return storageClient;
}

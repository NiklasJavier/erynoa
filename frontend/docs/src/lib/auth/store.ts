  ...
  return {
    ...
    async init(): Promise<void> {
      ...
      try {
        ...
        console.log('[AuthStore] OIDC initialized with clientId:', config.auth.clientId, 'redirectUri:', `${config.urls.docs}/callback`);
    ...
    async handleCallback(): Promise<User> {
      ...
      try {
        ...
        // Verwende Docs-URL aus Config für exakte Redirect-URI-Übereinstimmung mit Zitadel
        initAuth(config.auth.issuer, config.auth.clientId, config.urls.docs);
        console.log('[AuthStore] OIDC initialized for callback with clientId:', config.auth.clientId, 'redirectUri:', `${config.urls.docs}/callback`);

...
import * as Dialog from '@erynoa/ui/components/dialog'
  ...
  <PageContent>
      ...
      <Card.Header>
        <Card.Title class="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onclick={loadBuckets}>
            <RefreshCw
              class={`h-4 w-4 ${isLoadingBuckets ? "animate-spin" : ""}`}
            />
          </Button>
        </Card.Title>
      </Card.Header>
      ...
      <Card.Header>
        <Card.Title class="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onclick={loadObjects}>
            <RefreshCw
              class={`h-4 w-4 ${isLoadingObjects ? "animate-spin" : ""}`}
            />
          </Button>
        </Card.Title>
      </Card.Header>
        ...
        <div class="py-4">
          ...
          {#if selectedFile}
            <p class="text-sm text-muted-foreground mt-2">
              Selected: {selectedFile.name} ({formatFileSize(
                selectedFile.size,
              )})
            </p>
          {/if}
    ...
    <Dialog.Root bind:open={createBucketDialogOpen}>
      <Dialog.Content>
        ...
        <div class="py-4">
          <Input placeholder="bucket-name" bind:value={newBucketName} />
        </div>
        <Dialog.Footer>
          <Button
            variant="outline"
            onclick={() => (createBucketDialogOpen = false)}
          >
            Cancel
          </Button>

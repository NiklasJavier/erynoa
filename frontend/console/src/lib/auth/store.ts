...
function createAuthStore() {
	...
	return {
		...
		async init(): Promise<void> {
			...
			let state: AuthStoreState | undefined
			...
			try {
				...
				try {
					...
					console.log(
						'[AuthStore] User check complete:',
						user?.profile?.preferred_username || 'none'
					)
				} catch (userError) {
		...
		async handleCallback(): Promise<User> {
			...
			try {
				...
				console.log(
					'[AuthStore] User state updated successfully, isAuthenticated should be true now'
				)
				return user
			} catch (error) {
...
export const isAuthenticated: Readable<boolean> = derived(authStore, ($auth) => {

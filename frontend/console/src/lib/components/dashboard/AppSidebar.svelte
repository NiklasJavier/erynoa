...
{#snippet renderNavEntry(
  ...
  {#if hasChildren(entry)}
    ...
    {:else}
      ...
      <Collapsible.Root
        open={entry.defaultOpen || isChildActiveRecursive(entry)}
  ...
  <Sidebar.Content>
    ...
    {#each groups as group, i}
      {#if i > 0 || topItems.length > 0}
        ...
        <div
          class="mx-2 my-1 hidden h-px bg-sidebar-border group-data-[collapsible=icon]:block"
        ></div>
      {/if}
      <Sidebar.Group>
